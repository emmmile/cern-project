...
//        FILE *fout;
        ...
        for(theEventN=firstevent; theEventN<=TotalNEvents; theEventN++) {
            ...
            //int counter = 0;
            peakanalyzer p( detector + std::to_string((long long int)id) + " event " + std::to_string((long long int)theEventN) );
...
//            }
      ...
      printf( "%d %f %d %f\n", first_channel, channelwidth,theRunReader->GetSampleRate(), 1./theRunReader->GetSampleRate()*1000. );
            ...
            //cout << std::this_thread::get_id() << " Stream size was " << counter << endl;
            //getchar();
            //if ( theEventN == 1 ) {
